...
function Row({ icon: Icon, label, to, onClick, iconBg = '#F2F2F7', iconColor = '#1C1C1E', trailing, danger }: RowProps) {
  const content = (
    <div className="flex items-center gap-3 px-4 py-3.5 active:bg-slate-100 transition-colors">
      <div
        className="w-9 h-9 rounded-lg flex items-center justify-center shrink-0"
        style={{ backgroundColor: danger ? '#FEE2E2' : iconBg }}
      >
        <Icon className="w-5 h-5" style={{ color: danger ? '#DC2626' : iconColor }} strokeWidth={2} />
      </div>
      <span
      ...
      {trailing}
      {!trailing && !danger && <ChevronRight className="w-5 h-5 text-slate-400" strokeWidth={2} />}
    </div>
  );
...
export default function Profile() {
  ...
  const totalDownloads = myApps.reduce((s: number, a: any) => s + (a.downloads || 0), 0);
  const ratedApps = myApps.filter((a: any) => (a.rating || 0) > 0);
  const avgRating = ratedApps.length
    ? (ratedApps.reduce((s: number, a: any) => s + (a.rating || 0), 0) / ratedApps.length).toFixed(1)
    : '0.0';
  const savedCount = myApps.length;
        ...
        {!isAuthenticated ? (
          <div className="bg-white rounded-2xl p-8 text-center">
            ...
            <div className="flex flex-col gap-2">
              <Link
                to="/login"
                className="inline-flex items-center justify-center gap-2 h-11 rounded-full text-white font-semibold text-sm"
              ...
              <Link
                to="/register"
                className="inline-flex items-center justify-center gap-2 h-11 rounded-full bg-slate-100 font-semibold text-sm text-slate-800"
              >
          ...
          <>
            ...
            >
              ...
              <p className="text-sm text-slate-500 mt-0.5 flex items-center justify-center gap-1.5">
            ...
            <div className="grid grid-cols-3 gap-2.5 mt-4">
              <StatCard label="Rating" value={avgRating} Icon={Star} />
              <StatCard label="Saved" value={savedCount} Icon={Bookmark} />
              <StatCard label="Downloads" value={formatNum(totalDownloads)} Icon={DownloadCloud} />
            </div>

            {/* ACCOUNT */}
            <SectionTitle>Account</SectionTitle>
            <div className="bg-white rounded-2xl overflow-hidden divide-y divide-slate-100">
              <Row icon={UserIcon} label="Personal Information" onClick={() => setEditOpen(true)} />
            ...
            <div className="bg-white rounded-2xl overflow-hidden divide-y divide-slate-100">
              <Row icon={Globe} label="Language" onClick={() => toast.info('Coming soon')} trailing={<span className="text-sm text-slate-400 mr-1">English</span>} />
              <Row icon={HelpCircle} label="Help & Support" onClick={() => toast.info('Contact support@elorax.app')} />
              <Row icon={Info} label="About EloraX" onClick={() => toast.info('EloraX v1.0')} />
            </div>
            ...
            <div className="bg-white rounded-2xl overflow-hidden divide-y divide-slate-100">
              {!developerProfile && (
                <Row icon={Code2} label="Become a Developer" to="/developer/register" iconBg="#E0EFFF" iconColor={ACCENT} />
              )}
            ...
            {isAdmin && (
              <>
                ...
                <div className="rounded-2xl overflow-hidden" style={{ backgroundColor: '#F3EEFF' }}>
                  <Row icon={Shield} label="Admin Panel" to="/admin" iconBg="#E9DEFF" iconColor="#7C3AED" />
                </div>
              </>
            )}
        ...
        >
          ...
          >
            ...
            <div className="flex gap-2 mt-5">
              <button
                onClick={() => setEditOpen(false)}
                className="flex-1 h-11 rounded-full bg-slate-100 font-semibold text-sm text-slate-800"
              >
                Cancel
              </button>
              <button
